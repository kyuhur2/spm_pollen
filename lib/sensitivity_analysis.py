...
class ModelQAIC:
    ...
    def powerset_generator(self, confounding):
        for i in chain.from_iterable(
            combinations(confounding, j) for j in range(len(confounding) + 1)
        ):
            yield list(i)
    ...
    def best_model(self, data):
        ...
        for conf in self.powerset_generator(self.confounding):
            ...
            # calculate aic
            model1, model2, qaic1, qaic2 = self.interactive_qaic(data, eq1, eq2)
        ...
        self.export_data(path=path / "model/qaics.csv", data=qaics)
